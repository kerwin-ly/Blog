...
以上是`依赖注入`的简单实现，`Angular`的实现远比这个复杂。按模块进行注入、注入方式支持`值，工厂，类`、环形依赖判断等等。这里能力有限，就不去分析了。感兴趣的可以研究下这块的实现源码。https://github.com/angular/angular/blob/master/packages/core/src/di/injector.ts
